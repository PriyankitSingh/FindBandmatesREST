...
public class Auditor implements ContainerRequestFilter {
	...
	public void filter(ContainerRequestContext cxt) throws IOException {
		...
		Musician musician = fetchMusician(em, username);
	...
	// TODO: testing
	protected Musician fetchMusician(EntityManager em, String username){
		Musician musician = null;
		try {
			_logger.debug("Querying the database for the current user");
			TypedQuery<Musician> query = em.createQuery(
				"select u from User u where u._username = :username", Musician.class
				).setParameter("username", username);
			musician = query.getSingleResult();
			...
			musician = new Musician(username);
		}
		return musician;
	}
